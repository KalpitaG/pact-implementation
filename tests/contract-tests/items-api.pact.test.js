...
describe('Items API Contract', () => {
    ...
    test('search for items with a valid query', async () => {
        provider
            .given('items exist matching the search query')
            .uponReceiving('a request to search for items with a valid query')
            .withRequest({
                method: 'GET',
                path: '/items/search',
    ...
    test('create a new item', async () => {
        const newItem = { name: 'New Item', price: 25.00, category: 'Tools', inStock: true };
